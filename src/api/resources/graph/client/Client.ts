...
export class Graph {
     ...
     *     })
     */
    public async add(request: Zep.AddDataRequest, requestOptions?: Graph.RequestOptions): Promise<Zep.Episode> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.ZepEnvironment.Default,
                "graph",
    ...
    public async clone(
        ...
        requestOptions?: Graph.RequestOptions,
    ): Promise<Zep.CloneGraphResponse> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.ZepEnvironment.Default,
                "graph/clone",
            ...
            contentType: "application/json",
            requestType: "json",
            body: serializers.CloneGraphRequest.jsonOrThrow(request, { unrecognizedObjectKeys: "strip" }),
            timeoutMs: requestOptions?.timeoutInSeconds != null ? requestOptions.timeoutInSeconds * 1000 : 60000,
            maxRetries: requestOptions?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
        });
        if (_response.ok) {
            return serializers.CloneGraphResponse.parseOrThrow(_response.body, {
                unrecognizedObjectKeys: "passthrough",
                allowUnrecognizedUnionMembers: true,
                allowUnrecognizedEnumValues: true,
                skipValidation: true,
