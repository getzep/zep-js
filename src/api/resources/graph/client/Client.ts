...
export class Graph {
     ...
     *
     * @param {Zep.GraphitiAddNodesRequest} request
     * @param {Graph.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Zep.BadRequestError}
     * @throws {@link Zep.InternalServerError}
    ...
    public addNodes(
        ...
        requestOptions?: Graph.RequestOptions,
    ): core.HttpResponsePromise<Zep.GraphitiAddNodesResponse> {
        return core.HttpResponsePromise.fromPromise(this.__addNodes(request, requestOptions));
    }

    private async __addNodes(
        request: Zep.GraphitiAddNodesRequest,
        requestOptions?: Graph.RequestOptions,
    ): Promise<core.WithRawResponse<Zep.GraphitiAddNodesResponse>> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
            ...
            contentType: "application/json",
            requestType: "json",
            body: serializers.GraphitiAddNodesRequest.jsonOrThrow(request, {
                unrecognizedObjectKeys: "strip",
                omitUndefined: true,
            }),
            timeoutMs: requestOptions?.timeoutInSeconds != null ? requestOptions.timeoutInSeconds * 1000 : 60000,
        ...
        if (_response.ok) {
            return {
                data: serializers.GraphitiAddNodesResponse.parseOrThrow(_response.body, {
                    unrecognizedObjectKeys: "passthrough",
                    allowUnrecognizedUnionMembers: true,
                    allowUnrecognizedEnumValues: true,
                    skipValidation: true,
        ...
        switch (_response.error.reason) {
            ...
            case "timeout":
                throw new errors.ZepTimeoutError("Timeout exceeded when calling POST /graph/nodes.");
            case "unknown":
                throw new errors.ZepError({
                    message: _response.error.errorMessage,
                    rawResponse: _response.rawResponse,
