...
export class Episode {
     ...
     */
    public async delete(uuid: string, requestOptions?: Episode.RequestOptions): Promise<Zep.SuccessResponse> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.ZepEnvironment.Default,
                `graph/episodes/${encodeURIComponent(uuid)}`,
     ...
     */
    public async getNodesAndEdges(uuid: string, requestOptions?: Episode.RequestOptions): Promise<Zep.EpisodeMentions> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.ZepEnvironment.Default,
        ...
        if (_response.ok) {
            return serializers.EpisodeMentions.parseOrThrow(_response.body, {
                unrecognizedObjectKeys: "passthrough",
                allowUnrecognizedUnionMembers: true,
                allowUnrecognizedEnumValues: true,
                skipValidation: true,
