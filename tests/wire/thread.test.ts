// This file was auto-generated by Fern from our API Definition.

import * as Zep from "../../src/api/index";
import { ZepClient } from "../../src/Client";
import { mockServerPool } from "../mock-server/MockServerPool";

describe("ThreadClient", () => {
    test("list_all (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            response_count: 1,
            threads: [
                {
                    created_at: "created_at",
                    project_uuid: "project_uuid",
                    thread_id: "thread_id",
                    user_id: "user_id",
                    uuid: "uuid",
                },
            ],
            total_count: 1,
        };

        server.mockEndpoint().get("/threads").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.thread.listAll({
            pageNumber: 1,
            pageSize: 1,
            orderBy: "order_by",
            asc: true,
        });
        expect(response).toEqual({
            responseCount: 1,
            threads: [
                {
                    createdAt: "created_at",
                    projectUuid: "project_uuid",
                    threadId: "thread_id",
                    userId: "user_id",
                    uuid: "uuid",
                },
            ],
            totalCount: 1,
        });
    });

    test("list_all (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server.mockEndpoint().get("/threads").respondWith().statusCode(400).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.thread.listAll();
        }).rejects.toThrow(Zep.BadRequestError);
    });

    test("list_all (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server.mockEndpoint().get("/threads").respondWith().statusCode(500).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.thread.listAll();
        }).rejects.toThrow(Zep.InternalServerError);
    });

    test("create (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { thread_id: "thread_id", user_id: "user_id" };
        const rawResponseBody = {
            created_at: "created_at",
            project_uuid: "project_uuid",
            thread_id: "thread_id",
            user_id: "user_id",
            uuid: "uuid",
        };

        server
            .mockEndpoint()
            .post("/threads")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.thread.create({
            threadId: "thread_id",
            userId: "user_id",
        });
        expect(response).toEqual({
            createdAt: "created_at",
            projectUuid: "project_uuid",
            threadId: "thread_id",
            userId: "user_id",
            uuid: "uuid",
        });
    });

    test("create (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { thread_id: "thread_id", user_id: "user_id" };
        const rawResponseBody = {};

        server
            .mockEndpoint()
            .post("/threads")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.create({
                threadId: "thread_id",
                userId: "user_id",
            });
        }).rejects.toThrow(Zep.BadRequestError);
    });

    test("create (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { thread_id: "thread_id", user_id: "user_id" };
        const rawResponseBody = {};

        server
            .mockEndpoint()
            .post("/threads")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.create({
                threadId: "thread_id",
                userId: "user_id",
            });
        }).rejects.toThrow(Zep.InternalServerError);
    });

    test("delete (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { message: "message" };

        server
            .mockEndpoint()
            .delete("/threads/threadId")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.thread.delete("threadId");
        expect(response).toEqual({
            message: "message",
        });
    });

    test("delete (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server
            .mockEndpoint()
            .delete("/threads/threadId")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.delete("threadId");
        }).rejects.toThrow(Zep.NotFoundError);
    });

    test("delete (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server
            .mockEndpoint()
            .delete("/threads/threadId")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.delete("threadId");
        }).rejects.toThrow(Zep.InternalServerError);
    });

    test("get_user_context (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { context: "context" };

        server
            .mockEndpoint()
            .get("/threads/threadId/context")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.thread.getUserContext("threadId", {
            templateId: "template_id",
        });
        expect(response).toEqual({
            context: "context",
        });
    });

    test("get_user_context (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server
            .mockEndpoint()
            .get("/threads/threadId/context")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.getUserContext("threadId");
        }).rejects.toThrow(Zep.NotFoundError);
    });

    test("get_user_context (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server
            .mockEndpoint()
            .get("/threads/threadId/context")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.getUserContext("threadId");
        }).rejects.toThrow(Zep.InternalServerError);
    });

    test("get (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            messages: [
                {
                    content: "content",
                    created_at: "created_at",
                    metadata: { key: "value" },
                    name: "name",
                    processed: true,
                    role: "norole",
                    uuid: "uuid",
                },
            ],
            row_count: 1,
            thread_created_at: "thread_created_at",
            total_count: 1,
            user_id: "user_id",
        };

        server
            .mockEndpoint()
            .get("/threads/threadId/messages")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.thread.get("threadId", {
            limit: 1,
            cursor: 1,
            lastn: 1,
        });
        expect(response).toEqual({
            messages: [
                {
                    content: "content",
                    createdAt: "created_at",
                    metadata: {
                        key: "value",
                    },
                    name: "name",
                    processed: true,
                    role: "norole",
                    uuid: "uuid",
                },
            ],
            rowCount: 1,
            threadCreatedAt: "thread_created_at",
            totalCount: 1,
            userId: "user_id",
        });
    });

    test("get (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server
            .mockEndpoint()
            .get("/threads/threadId/messages")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.get("threadId");
        }).rejects.toThrow(Zep.NotFoundError);
    });

    test("get (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server
            .mockEndpoint()
            .get("/threads/threadId/messages")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.get("threadId");
        }).rejects.toThrow(Zep.InternalServerError);
    });

    test("add_messages (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { messages: [{ content: "content", role: "norole" }] };
        const rawResponseBody = { context: "context", message_uuids: ["message_uuids"], task_id: "task_id" };

        server
            .mockEndpoint()
            .post("/threads/threadId/messages")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.thread.addMessages("threadId", {
            messages: [
                {
                    content: "content",
                    role: "norole",
                },
            ],
        });
        expect(response).toEqual({
            context: "context",
            messageUuids: ["message_uuids"],
            taskId: "task_id",
        });
    });

    test("add_messages (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {
            messages: [
                { content: "content", role: "norole" },
                { content: "content", role: "norole" },
            ],
        };
        const rawResponseBody = {};

        server
            .mockEndpoint()
            .post("/threads/threadId/messages")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.addMessages("threadId", {
                messages: [
                    {
                        content: "content",
                        role: "norole",
                    },
                    {
                        content: "content",
                        role: "norole",
                    },
                ],
            });
        }).rejects.toThrow(Zep.InternalServerError);
    });

    test("add_messages_batch (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { messages: [{ content: "content", role: "norole" }] };
        const rawResponseBody = { context: "context", message_uuids: ["message_uuids"], task_id: "task_id" };

        server
            .mockEndpoint()
            .post("/threads/threadId/messages-batch")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.thread.addMessagesBatch("threadId", {
            messages: [
                {
                    content: "content",
                    role: "norole",
                },
            ],
        });
        expect(response).toEqual({
            context: "context",
            messageUuids: ["message_uuids"],
            taskId: "task_id",
        });
    });

    test("add_messages_batch (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {
            messages: [
                { content: "content", role: "norole" },
                { content: "content", role: "norole" },
            ],
        };
        const rawResponseBody = {};

        server
            .mockEndpoint()
            .post("/threads/threadId/messages-batch")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.addMessagesBatch("threadId", {
                messages: [
                    {
                        content: "content",
                        role: "norole",
                    },
                    {
                        content: "content",
                        role: "norole",
                    },
                ],
            });
        }).rejects.toThrow(Zep.InternalServerError);
    });

    test("get_summary (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            created_at: "created_at",
            last_summarized_at: "last_summarized_at",
            summary: "summary",
            thread_id: "thread_id",
            uuid: "uuid",
        };

        server
            .mockEndpoint()
            .get("/threads/threadId/summary")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.thread.getSummary("threadId");
        expect(response).toEqual({
            createdAt: "created_at",
            lastSummarizedAt: "last_summarized_at",
            summary: "summary",
            threadId: "thread_id",
            uuid: "uuid",
        });
    });

    test("get_summary (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server
            .mockEndpoint()
            .get("/threads/threadId/summary")
            .respondWith()
            .statusCode(403)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.getSummary("threadId");
        }).rejects.toThrow(Zep.ForbiddenError);
    });

    test("get_summary (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server
            .mockEndpoint()
            .get("/threads/threadId/summary")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.getSummary("threadId");
        }).rejects.toThrow(Zep.NotFoundError);
    });

    test("get_summary (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ZepClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {};

        server
            .mockEndpoint()
            .get("/threads/threadId/summary")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.thread.getSummary("threadId");
        }).rejects.toThrow(Zep.InternalServerError);
    });
});
